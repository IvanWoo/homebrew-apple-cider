  ...
  <div class="Albumlist">
    <div class="container-fluid">
      <ul class="media-list row">
        <li
          class="media row col-sm-12 col-lg-6"
          v-for="(album, index) in albums"
          :key="album.id"
        >
          <div class="col-sm-12 col-md-4">
            <img
              class="media-left media-object mw-100"
              v-bind:src="album.coverBig"
            />
          </div>
...
export default {
  ...
  methods: {
    getAlbumList: function (source) {
      axios
        .get(
          'https://douting.leanapp.cn/api/search/album/netease?&limit=3&key=' +
            source
        )
        .then((response) => {
